...
mutable struct Traffic_model2 <: AbstractEnv
    s::SVector{3, Int}
end

function CommonRLInterface.reset!(m::Traffic_model2)
    m.s = SA[2, 2, 0]
end

# Additional commands
    CommonRLInterface.actions(m::Traffic_model2) = (:NS, :EW)
    CommonRLInterface.observe(m::Traffic_model2) = m.s
    CommonRLInterface.terminated(m::Traffic_model2) = false
    CommonRLInterface.observations(env::Traffic_model2) = [SA[x, y, i] for x in 0:50, y in 0:50, i in 0:1]

function CommonRLInterface.act!(m::Traffic_model2, a)
...
# Optional functions can be added like this:
CommonRLInterface.clone(m::Traffic_model2) = Traffic_model(m.s)
...
# Now redo but with the modified traffic model
env = convert(AbstractEnv, Traffic_model2([5, 5, 0]))
...
plot!(p, 1:cycle_times, NS_flows_Learn, lw = 1.5, ls = :solid, color = 1, label = "Reinforcement Learned")
plot!(p, 1:cycle_times, NS_flows_conv, lw = 1, ls = :dash, color = 1, label = "Conventional Planning")
savefig(p, "LearnedvsConv_Flows_NS_robust.png")

p = plot(title="Learned vs Conventional Signals: East/Westbound", xlim=(50, 150))
plot!(p, 1:cycle_times, EW_flows_Learn, lw = 1.5, ls = :solid, color = 2, label = "Reinforcement Learned")
plot!(p, 1:cycle_times, EW_flows_conv, lw = 1, ls = :dash, color = 2, label = "Conventional Planning")
savefig(p, "LearnedvsConv_Flows_EW_robust.png")
